...
public class RentalException extends Exception {
  ...
  private static final long serialVersionUID = 928785036748596097L;
  ...
  public enum RentalErrors {
    ...
    EMAIL_NOT_EXISTS(
        6, "The e-mail does not exist in database", "error.user.not.exists", "/register/resend"),
    TOKEN_NOT_EXISTS(
        7, "The token does not exist in database", "error.token.not.exists", "/register/resend"),
    INVOICE_NOT_AVAILABLE(
        8, "Requested invoice is not available.", "error.invoice.not.available", null),
    ...
    USER_ALREADY_ACTIVE(
        12, "User connected with this email is already active", "error.user.active", "/login");
